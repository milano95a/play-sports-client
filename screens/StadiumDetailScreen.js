...
export default class StadiumDetailScreen extends React.Component {
  ...
  componentWillMount() {
    ...
    this.setState({reviews, status: this.props.navigation.state.params.stadium.status});
  ...
  renderReviewList(){
    ...
    : this.state.reviews.map((r, i) => (
        <ListItem
          ...
          title={`Review from ${this.giveMeName(r.reviewer)}`}
          subtitle={
  ...
  render(){
    ...
    const statusText = this.state.status ? 'Open' : 'Closed';
              ...
              <Button
                ...
                onPress={() => {
                  this.props.navigation.navigate(
                    'BookScreen',
                    {stadium : this.props.navigation.state.params.stadium}
                  );
                }}>
                Book
              </Button>
